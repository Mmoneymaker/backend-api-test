"""
GAIA Benchmark LangSmith 评估测试 - PlanningFlow 版本
使用 GAIA 数据集测试 PlanningFlow Agent 的性能，与 Manus 进行对比
"""
import asyncio
import os
import sys
...
# ============================================================
# GAIA 数据加载（与 test_gaia_langsmith.py 相同）
# ============================================================
...
def prepare_gaia_examples(level: str = "level1") -> List[Dict]:
    """加载 GAIA 数据集（与 test_gaia_langsmith.py 相同）"""
    import pandas as pd
...
def _get_tool_calls_from_agent(agent) -> List[Dict]:
    """从 Agent 中获取工具调用记录"""
    tool_calls = []
    if agent is None:
        return tool_calls

    # 尝试从 memory.messages 获取
    ...
    # 尝试从 messages 属性获取
    if not tool_calls and hasattr(agent, 'messages'):
...
def _last_assistant_content(agent) -> str:
...
def _get_thinking_process(agent) -> str:
    """获取 Agent 的思考过程（保留此函数用于兼容性）"""
    thoughts = []
    if agent is None:
        return ""
    ...
    # 如果 memory 为空，尝试直接访问 messages 属性
    if not thoughts and hasattr(agent, 'messages'):
        for msg in agent.messages:
            if hasattr(msg, "role") and msg.role == "assistant":
                content = getattr(msg, "content", None)
                if content:
...
def _get_thinking_from_execution_history(flow) -> str:
    """从 PlanningFlow 的 execution_history 获取思考过程"""
    if not hasattr(flow, 'execution_history') or not flow.execution_history:
        return ""

    thoughts = []
    for entry in flow.execution_history:
...
def _get_tool_calls_from_execution_history(flow) -> List[Dict]:
    """从 PlanningFlow 的 execution_history 获取工具调用记录"""
    if not hasattr(flow, 'execution_history') or not flow.execution_history:
        return []
    ...
    for entry in flow.execution_history:
        ...
        for msg in messages:
            ...
            if tool_calls_in_msg:
                tool_calls.extend(tool_calls_in_msg)

    return tool_calls


def _get_step_info(flow) -> Dict[str, Any]:
    """获取计划的步骤信息"""
    if hasattr(flow, 'planning_tool') and hasattr(flow.planning_tool, 'plans'):
        plan_id = flow.active_plan_id
        if plan_id in flow.planning_tool.plans:
            plan = flow.planning_tool.plans[plan_id]
            return {
...
# ============================================================
# Step 1: 定义应用（使用与 test_gaia_langsmith.py 相同的项目名）
# ============================================================

@traceable(name="gaia-planning-flow-agent", project_name=os.getenv("LANGSMITH_PROJECT", "manus-gaia-evaluation"))
async def gaia_planning_flow_application(inputs: Dict[str, Any]) -> Dict[str, Any]:
    """
    GAIA Benchmark PlanningFlow 测试应用
    返回完整的执行信息供评估，与 Manus 版本对比
    """
    ...
    # 保存 primary_agent 的引用，用于后续获取 thinking
    primary_agent = flow.primary_agent

    try:
        await flow.execute(full_query)

        # 获取完整信息（在 cleanup 之前）
        # 从 flow.execution_history 获取 tool_calls（不会被 cleanup 清除）
        tool_calls = _get_tool_calls_from_execution_history(flow)
        tool_names = [tc["name"] for tc in tool_calls]
        tool_calls_count = len(tool_calls)
        output = _last_assistant_content(primary_agent)
        # 注意：此时 primary_agent 可能已经被 cleanup 了，尝试获取 thinking
        # 从 flow.execution_history 获取 thinking（不会被 cleanup 清除）
        thinking = _get_thinking_from_execution_history(flow)

        # 获取步骤信息
        step_info = _get_step_info(flow)
        steps = step_info.get("steps", [])
        step_statuses = step_info.get("step_statuses", [])
        step_count = step_info.get("step_count", 0)

        # 获取 Token 使用
        prompt_tokens = 0
        completion_tokens = 0
        if hasattr(primary_agent, 'llm') and primary_agent.llm:
            prompt_tokens = getattr(primary_agent.llm, "total_input_tokens", 0)
            completion_tokens = getattr(primary_agent.llm, "total_completion_tokens", 0)
        ...
        # 清理资源（如果还没有被 cleanup 的话）
        if primary_agent and hasattr(primary_agent, 'state'):
            try:
                await primary_agent.cleanup()
            except Exception:
                pass  # 可能已经被 cleanup 了


# ============================================================
# Step 2: 创建数据集（与 test_gaia_langsmith.py 相同）
# ============================================================

def create_gaia_dataset(level: str = "level1", dataset_name: str = None) -> str:
    """创建 GAIA 数据集（与 test_gaia_langsmith.py 相同）"""
    ls_client = Client()

    # 使用与 test_gaia_langsmith.py 相同的数据集名称，便于对比
    ...
    dataset = ls_client.create_dataset(
        ...
        description=f"GAIA Benchmark - {level} - PlanningFlow Agent Evaluation"
    )
...
# ============================================================
# Step 3: 评估器（与 test_gaia_langsmith.py 相同 + PlanningFlow 特有）
# ============================================================

def answer_contains_reference(inputs: dict, outputs: dict, reference_outputs: dict) -> dict:
    """评估回答是否包含标准答案（与 test_gaia_langsmith.py 相同）"""
...
def has_tool_calls(inputs: dict, outputs: dict, reference_outputs: dict) -> dict:
    """评估是否有工具调用（与 test_gaia_langsmith.py 相同）"""
...
def tool_call_efficiency(inputs: dict, outputs: dict, reference_outputs: dict) -> dict:
    """评估工具调用效率（与 test_gaia_langsmith.py 相同）"""
    tool_calls_count = outputs.get("tool_calls_count", 0)
...
def has_output(inputs: dict, outputs: dict, reference_outputs: dict) -> dict:
    """评估是否有有效输出（与 test_gaia_langsmith.py 相同）"""
...
# PlanningFlow 特有评估器
def plan_created(inputs: dict, outputs: dict, reference_outputs: dict) -> dict:
    """评估是否成功生成了计划"""
    step_count = outputs.get("step_count", 0)
    has_plan = step_count > 0

    return {
        "key": "plan_created",
        "score": 1.0 if has_plan else 0.0,
        "reason": f"Step count: {step_count}"
    }


# ============================================================
# Step 4: LLM 评估器（与 test_gaia_langsmith.py 类似）
# ============================================================

def create_llm_judge_evaluator():
    """创建基于 LLM 的评估器 - 单次调用，多维度评估"""
    ...
    async def llm_judge_evaluator(inputs: dict, outputs: dict, reference_outputs: dict) -> dict:
        ...
        step_statuses = outputs.get("step_statuses", [])
        step_count = outputs.get("step_count", 0)

        evaluation_prompt = f"""你是一个严格的评估专家。请从以下维度评估 PlanningFlow Agent 在 GAIA Benchmark 上的表现：
...
## PlanningFlow 计划信息
步骤总数: {step_count}
步骤详情: {json.dumps(list(zip(steps, step_statuses)), ensure_ascii=False, indent=2) if steps else "无"}
...
4. **计划执行效果 (planning_effectiveness)**: PlanningFlow 的计划执行是否有效？是否体现了计划驱动的优势？
   - 1分：计划合理、执行有效、步骤清晰可控
   - 0.5分：有一定计划但效果一般
   - 0分：计划执行混乱或与直接执行无区别
...
{{
    ...
    "planning_effectiveness": <0-1之间的分数>,
    "overall_score": <上述四项的平均分>,
    "reason": "<简要说明评估理由>"
}}
...
注意：只返回 JSON，不要有其他内容。"""
        ...
        try:
            ...
            planning_effectiveness = float(result.get("planning_effectiveness", 0.5))
            overall = float(result.get("overall_score", (task_completion + tool_usage + answer_quality + planning_effectiveness) / 4))

            reason = result.get("reason", "")
            detailed_reason = f"Task: {task_completion}, Tool: {tool_usage}, Quality: {answer_quality}, Planning: {planning_effectiveness}. {reason}"
...
async def run_evaluation(
    ...
    """运行 GAIA 评估（与 test_gaia_langsmith.py 相同的数据集）"""
    ls_client = Client()

    # 创建或获取数据集（与 test_gaia_langsmith.py 相同）
    if dataset_name is None:
        dataset_name = f"GAIA_Manus_{level}"
    ...
    # 准备评估器（与 test_gaia_langsmith.py 相同 + plan_created）
    evaluators = [
        answer_contains_reference,
        has_tool_calls,
        tool_call_efficiency,
        has_output,
        plan_created,  # PlanningFlow 特有
    ]
    ...
    if use_llm_judge:
        ...
        if llm_judge:
            ...
            print("已添加 LLM 评估器 (task_completion, tool_usage, answer_quality, planning_effectiveness)")

    # 运行评估
    results = await ls_client.aevaluate(
        gaia_planning_flow_application,
        data=dataset_name,
        evaluators=evaluators,
        experiment_prefix=f"planning-flow-gaia-{level}",  # 与 Manus 版本区分
        description=f"GAIA Benchmark - {level} - PlanningFlow Agent",
        max_concurrency=2,
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(description="GAIA Benchmark LangSmith 评估测试 - PlanningFlow 版本")
    parser.add_argument(
        "--mode",
        type=str,
